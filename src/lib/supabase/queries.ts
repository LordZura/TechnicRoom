import { Locale, ProductListItem, ProductWithRelations } from '@/types';
import { createSupabaseServerClient } from './server';

export async function getProducts(search?: string, includeInactive = false): Promise<ProductListItem[]> {
  const supabase = createSupabaseServerClient();
  let query = supabase.from('products_search').select('*').order('created_at', { ascending: false });
