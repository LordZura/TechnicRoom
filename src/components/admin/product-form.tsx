...
export function ProductForm({ initialData }: { initialData?: Partial<ProductFormInput> }) {
  const [message, setMessage] = useState('');
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<ProductFormInput>({
  ...
  const fields = [
    'slug','model','brand','category','recommended_area','cooling_power','heating_power','cooling_consumption','heating_consumption','eer_cop','freon_type_amount','operating_temperature','indoor_unit_size','indoor_unit_weight','outdoor_unit_size','outdoor_unit_weight','noise_level','pipe_size'
  ] as const;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 rounded-xl border border-slate-200 bg-white p-6">
      ...
      <button disabled={isSubmitting} className="rounded-lg bg-brand-700 px-4 py-2 text-white">{isSubmitting ? 'Saving...' : 'Save Product'}</button>
