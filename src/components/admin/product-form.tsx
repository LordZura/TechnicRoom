      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {fields.map((field) => (
          <label key={field} className="space-y-1 text-sm">
            <span className="capitalize text-[#6b5442]">{field.replaceAll('_', ' ')}</span>
            <input className="tr-input" {...register(field)} />
            {errors[field] && <span className="text-xs text-red-700">Invalid value</span>}
          </label>
        ))}
        <label className="space-y-1 text-sm">
          <span>Price</span>
          <input type="number" step="0.01" className="tr-input" {...register('price')} />
        </label>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="space-y-1 text-sm">
          <span>English Name</span>
          <input className="tr-input" {...register('translations.0.name')} />
        </label>
        <label className="space-y-1 text-sm">
          <span>Georgian Name</span>
          <input className="tr-input" {...register('translations.1.name')} />
        </label>
        <label className="space-y-1 text-sm">
          <span>English Features</span>
          <textarea className="tr-input" {...register('translations.0.features')} />
        </label>
        <label className="space-y-1 text-sm">
          <span>Georgian Features</span>
          <textarea className="tr-input" {...register('translations.1.features')} />
        </label>
      </div>
        ...
        {productId && (
          <>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {images.map((image) => (
                <div key={image.id} className="space-y-2 rounded-xl border border-brand-line bg-brand-ivory p-2">
